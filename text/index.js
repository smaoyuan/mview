...
const options = {
  provider: 'google',
  apiKey: 'AIzaSyCW3I51tIwvHxHw-eyUsvJCgEiC8w_n9ng'
};
  ...
  xmlRead.readXML(txtData, function(err, data) {
    ...
    documentCollection.forEach(document => {
      ...
      document.children.forEach(ele => {
        ...
        if(ele.name === 'docID') {
          obj.docId = ele.children[0].value;
        }

        if(ele.name === 'docText') {
          obj.docText = ele.children[0].value;
        }
    ...
    geoRequest.then(r => {
      ...
      if(data) {
        mapDataCollection.push({
          ...
          docId: outputData[counter].docId,
          docText: outputData[counter].docText,
          location: outputData[counter].location
        });
      }
...
app.get("/getDocumentData", (req, res) => {
  xmlRead.readXML(txtData, function(err, data) {
    const ast = XmlReader.parseSync(data.content);
    const documentCollection = xmlQuery(ast).children().ast;
    const documentSortedData = [];
    ...
    documentCollection.forEach(document => {
      ...
      document.children.forEach(ele => {
        ...
        if(ele.name === 'Person') {
          obj.name.push(ele.children[0].value);
        }

        if(ele.name === 'Location') {
          obj.location.push(ele.children[0].value);
        }
      });

      documentSortedData.push(obj);
    });

    res.status(201).json({
      documentData: documentSortedData
    });
  });
});
